...
let captionMatches = text.match(/name: "([^"]+)"/g);
...
if (embedMatches && embedMatches.length > 0) {
	embedMatches.forEach(match => {
		const embedValue = match.match(/embed: "([^"]+)"/)[1];
		tabsWeb.push(embedValue);
	});
}
if (captionMatches && captionMatches.length > 0) {
	captionMatches.forEach(match => {
		const captionValue = match.match(/name: "([^"]+)"/)[1];
		tabsLeng.push(captionValue);
	});
}
// Coregir texto de array de idiomas
for (let i = 0; i < tabsLeng.length; i++) {
	let value = tabsLeng[i].toLowerCase();
	if (value.includes('español')) {tabsLeng[i] = 'Castellano';}
